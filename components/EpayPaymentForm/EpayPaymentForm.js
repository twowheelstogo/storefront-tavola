...
const renderIcons = (ccIcons) => (
  <div>
    {ccIcons.map((icon, index) => (
      <CardSpan key={index}>{icon}</CardSpan>
    ))}
  </div>
);
function EpayPaymentForm(props, ref) {
  ...
  let [dataCache, setDataCache] = useState({});
  ...
  const handleChange = (formData) => {
    ...
    if (isEqual(dataCache, formData)) return;
    setDataCache(formData);
    dataCache = formData;
    const resultDoc = buildResult(formData);
    const stringDoc = JSON.stringify(resultDoc);
    if (stringDoc !== lastDocRef.current) {
		// console.info("LOG: handleChange", onChange);
		// console.info("LOG: handleChange:resultDoc", resultDoc);
      onChange(resultDoc);
    }
  ...
  const { submitForm } = useReactoForm({
    isReadOnly: isSaving,
  ...
  const ccIcons = [props.components.iconVisa, props.components.iconMastercard];
  return (
    <div>
      <AcceptedPaymentMethods>{renderIcons(ccIcons)}</AcceptedPaymentMethods>
      <Form
        onSubmit={submitForm}
          ...
          return (
            <div onSubmit={handleSubmit}>
              <form className={className}>
                ...
                {/* <button onClick={(e) => handleChange(values, e)}>Saving</button> */}
              </form>
            </div>
          );
        }}
