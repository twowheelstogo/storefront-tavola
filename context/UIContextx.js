...
export const UIProvider = ({ children }) => {
  ...
  // const SelectedItems = (def) => {
  //   const id = selectedCartCatalog(def);
  //   return selectedItems[id];
  // };
  // const uSelectedItem = (opts) => {
  //   // variantId, optionId
  //   const id = selectedCartCatalog(opts.cartCatalogId);
  //   let key = `${opts.variantId}${opts.optionId ? `_${opts.optionId}` : ""}`;
  //   selectedItems[id][key] = opts.qty;
  //   setPdpSelectedItems(selectedItems);
  // };
  // const rSelectedItems = (opts) => {
  //   const id = selectedCartCatalog(opts.cartCatalogId);
  ...
  const selectedCartCatalog = (def) => {
    const id = def || selectedCartCatalogId;
    //
    if (!selectedCatalogs[id]) {
      selectedCatalogs[id] = { options: {}, qty: 1, xob: {} };
  ...
  const SelectedOptions = (def) => {
    const id = selectedCartCatalogId;//selectedCartCatalog(def);
  ...
  return (
    <UIContext.Provider
      value={{
        ...
        selectedCartCatalog,selectedCartCatalogF,
        selectedCatalogs,
